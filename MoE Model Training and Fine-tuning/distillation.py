...
def init_student_model(lm_config):
    ...
    model = MyModelForCausalLM(lm_config)
    moe_path = "_moe" if lm_config.use_moe else ""
    ckp = f"{args.save_dir}/pretrain_{lm_config.hidden_size}{moe_path}.pth"
    state_dict = torch.load(ckp, map_location=args.device)
    model.load_state_dict(state_dict, strict=False)
    Logger(f"Trainable parameters in the student model: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f} millions.")
    model = model.to(args.device)
    return model, tokenizer


def init_teacher_model(lm_config):
    # teacher will use the same tokenizer as the student's
    model = MyModelForCausalLM(lm_config)
    moe_path = "_moe" if lm_config.use_moe else ""
    ckp = f"{args.save_dir}/pretrain_{lm_config.hidden_size}{moe_path}.pth"
    state_dict = torch.load(ckp, map_location=args.device)
    model.load_state_dict(state_dict, strict=False)
    Logger(f"Trainable parameters in the teacher model: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f} millions.")
    model = model.to(args.device)
    return model
...
def train_epoch(epoch, alpha = 0.0, temperature=1.0):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        with ctx:
            res = model(X) # student model forward pass for prediction results
            students_logits = res.logits

        if teacher_model is not None:
            with torch.no_grad():
                teacher_logits = teacher_model(X).logits
                vocab_size_student = students_logits.size(-1)
                teacher_logits = teacher_logits[..., :vocab_size_student] # align the vocab size

        # calculate loss
        # 1. Ground-truth cross entropy loss
        loss_mask_flat = loss_mask.view(-1)
        ce_loss = F.cross_entropy(
            students_logits.view(-1, students_logits.size(-1)),
            Y.view(-1),
            ignore_index = 0,
            reduction = "none"
        )
        ...
        if teacher_model is not None:
            distill_loss = distillation_loss_fn(
                # only calculate the loss for the masked positions
                students_logits.view(-1, students_logits.size(-1))[loss_mask_flat==1],
        ...
        # 3. total_loss = alpha*CE + (1-alpha)*Distill
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=3) # for better training, use 2-6 epoches
    ...
    parser.add_argument("--data_path", type=str, default="./data/sft_min_512.jsonl") # use supervised learning dataset
    ...
    for epoch in range(args.epochs):
        train_epoch(epoch, alpha=0.5, temperature=2.0)
